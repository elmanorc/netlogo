...
;  set carga-eletron -1
  ...
  set potenciais ( list 7.625 60.00 60.00 )
  
  ; formas e questões de visualização
  set-default-shape atomos "circle"
  set-default-shape eletrons "circle"
  set-default-shape campos "simple-arrow"
  
  ask patches [
    sprout-campos 1 [ set color black]
  ]
...
to setup-campo-here
  let resultado intensidade-e-angulo
  let a-intensidade item 0 resultado
  let o-angulo item 1 resultado
  
  set heading o-angulo
  set intensidade-campo a-intensidade
  set color scale-color red intensidade-campo 0 100 
  ifelse abs intensidade-campo < 1e-10 or any? atomos-here
    [set hidden? true]
    [set hidden? false]
end
...
to-report intensidade-e-angulo
  let ace-int-x 0
  let ace-int-y 0
   foreach sort other ( turtle-set eletrons atomos ) [
     let dist distance ?
     if dist != 0
     [ 
       let intensidade-parcial ( k0 * [ carga ] of ? ) / (dist * dist) 
       let ang towards ?
       let int-x (  intensidade-parcial * sin ang )
       let int-y (  intensidade-parcial * cos ang )
       set ace-int-x ace-int-x + int-x
       set ace-int-y ace-int-y + int-y
     ]
   ]
  let a-intensidade hip ace-int-x ace-int-y
  let o-angulo 0
  if ace-int-x != 0 or ace-int-y != 0 
  [ set o-angulo atan ace-int-x ace-int-y print "15"]
  if abs a-intensidade < 1e-10 [ set a-intensidade 0 print "16"]
  report (list a-intensidade o-angulo)
end
@#$#@#$#@
GRAPHICS-WINDOW
2
...
true
0
Line -7500403 true 150 225 150 75
