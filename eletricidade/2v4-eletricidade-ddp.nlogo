...
to setup-terminais
  ask patches with [ ( pxcor < -42 or pxcor > 42) and pycor < 8 and pycor > -8 ] [
    sprout-terminais 1 [
      set color blue
      set eletrons-perdidos 0.1
      if pxcor > 0 
        [ set color red 
          set eletrons-perdidos -0.1
        ]
...
;  set carga-eletron -1
  ...
  set-default-shape terminais "square"
  ...
  ; eletrons perdidos dos terminais
...
to go-eletrons
  ...
  let ace-int-y 0
   ...
   foreach sort ( turtle-set other atomos terminais eletrons ) [ ;eletrons atomos ) [ - nesse modelo elétrons não interagem
     let dist distance ?
     if dist != 0
     [ 
       let intensidade ( -1 * k * [ carga ] of ? ) / (dist * dist) 
       face ?
       let ang towards ?
...
to go-terminais
  ask terminais [
    if xcor < 0 [
      ; let minha-alma-gemea one-of terminais with [ pxcor = ( - [ pxcor ] of myself ) and pycor = [ pycor ] of myself ]
      foreach sort eletrons-here [
        ask ? [ die ]
        ask min-one-of terminais with [ xcor > 0 ] [ xcor ] [ 
          hatch-eletrons 1 [
            ; set velocidade-x velo-saida
            set velocidade-x 0
            set velocidade-y 0
            set size 0.6
            set color yellow
...
true
0
Rectangle -7500403 true true 150 30 270 270
